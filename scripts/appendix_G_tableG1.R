...
# Coan, Holman (2023) -- Appendix Table G.1: Member of the House of
# Representatives.""
...
# Subset members of the House
data_house <- data[data$type == "REP", ]
...
# Subset Democrats
d_data <- data_house[data_house$party == "Democratic", ]
...
bformula <- bf(
  mask_prop_rescale ~ 1 + scale(nokken_poole_dim1) +  scale(nokken_poole_dim2) + scale(net_trump_vote) + scale(margin_pc) + gender + scale(age) + scale(cases_norm_l1) + scale(deaths_norm_l1) + mask_mandate + scale(dpres) + scale(medianIncomeE) + scale(age65_prop) + scale(Very.low.density) + scale(followers) + (1|bioguide) + (1|yr_wk),
  zi ~ 1 + scale(nokken_poole_dim1) +  scale(nokken_poole_dim2) + scale(net_trump_vote) + scale(margin_pc) + gender + scale(age) + scale(cases_norm_l1) + scale(deaths_norm_l1) + mask_mandate + scale(dpres) + scale(medianIncomeE) + scale(age65_prop) + scale(Very.low.density) + scale(followers) + (1|bioguide) + (1|yr_wk)
)
...
# data frame.
idx <- c(rep(1:30), idx_cdc, idx_biden, idx_trump, idx_elect)
...
# Subset Republicans
r_data <- data_house[data_house$party == "Republican", ]
...
write.csv(estimates_labels, "tables/appendix_G_tableG1.csv", row.names = FALSE)
